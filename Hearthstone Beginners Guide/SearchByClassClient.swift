...
class SearchByClassClient {
    ...
    static func searchCardsByClass(hero: Heroes?, location: Location,completionHandler: ([Card]?, NSError?) -> Void) -> Void {
        ...
        guard let url = NSURL(string: ParameterConstants.HEARTHSTONE_API_URI + ParameterConstants.HEARTHSTONE_API_CLASS_CARDS_ENDPOINT + heroQuery.rawValue + ParameterConstants.HEARTHSTONE_API_PARAMETER_LOCALE + locationQuery.rawValue) else{
            return;
        }
